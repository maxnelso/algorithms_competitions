...
for i in range(tests):
  ...
  for coord in vertices:
    ...
    for j in range(len(vertices)):
      for combination in itertools.combinations(vertices, j):
        if coord not in combination:
          new_vertices = []
          for c in vertices:
            if c not in combination:
              new_vertices.append([c.x, c.y])
          hull = ConvexHull(new_vertices, joggle = False)
          found = False
          for s in hull.simplices:
            simplex = s.coords
            if int(simplex[0][0]) == coord.x and \
               int(simplex[0][1]) == coord.y:
              found = True
          if found:
            out = j
            done = True
