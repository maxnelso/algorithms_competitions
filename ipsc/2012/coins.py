...
def parse_input(input_file):
    ...
    final_string = ""
    subset_function = lambda l: reduce(lambda z, x: z + [y + [x] for y in z], l, [[]])
    for i in range(test_cases):
        print i
        f.readline()
        num_coins = int(f.readline())
        coins = []
        for coin in f.readline().split():
            coins.append(float(coin))
        count = 0
        subsets = subset_function(coins)
        for subset in subsets:
            multiplication = 1
            for element in subset:
                multiplication = multiplication * element
            if len(subset)!=0 and multiplication==.5:
                count = count + 1
        if i!=test_cases-1:
