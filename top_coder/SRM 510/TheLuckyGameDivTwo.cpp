...
int TheLuckyGameDivTwo::find(int a, int b, int jLen, int bLen) {
  int lucky_count[5000];
  lucky_count[0] = 0;
  for (int i = 1; i < 5000; ++i) {
    lucky_count[i] = lucky_count[i - 1] + is_lucky(i);
  }
  int best = 0;
  for (int i = a; i <= b - jLen + 1; ++i) {
    int this_best = 100000;
    for (int j = i; j < i + jLen - bLen + 1; ++j) {
      this_best = min(this_best, lucky_count[j + bLen - 1] - lucky_count[j - 1]);
    }
