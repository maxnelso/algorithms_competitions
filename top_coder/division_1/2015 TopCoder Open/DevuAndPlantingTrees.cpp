    ...
    int maximumTreesDevuCanGrow(vector<string> garden) {
      ...
      for (int i = 0; i < garden[0].size(); i++) {
        if (garden[0][i] == '.' && garden[1][i] == '.') {
          bool good = true;
          if (i - 1 >= 0) {
            if (garden[0][i-1] == '*' || garden[1][i-1] == '*') {
              good = false;
            }
          }
          if (i + 1 < garden[0].size()) {
            if (garden[0][i+1] == '*' || garden[1][i+1] == '*') {
              good = false;
            }
          }
...
// CUT begin
ifstream data("DevuAndPlantingTrees.sample");
...
bool do_test(vector<string> garden, int __expected) {
