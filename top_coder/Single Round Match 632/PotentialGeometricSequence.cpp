...
class PotentialGeometricSequence {
  public: int numberOfSubsequences(vector<int> d);
};

bool is_good(vector<int> d) {
  if (d.size() == 1) {
    return true;
  }
  int diff = d[1] - d[0];
...
int PotentialGeometricSequence::numberOfSubsequences(vector<int> d) {
  ...
  for (int i = 0; i < d.size(); ++i) {
    for (int j = i; j < d.size(); ++j) {
      vector<int>::const_iterator first = d.begin() + i;
      vector<int>::const_iterator last = d.begin() + j + 1;
